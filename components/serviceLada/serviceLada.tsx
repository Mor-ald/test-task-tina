	...
	return (
		<Section color={"tint"}>
			<div className="my-12 mx-24 flex flex-col">
				{!data.serviceLada.header || <p data-testid="header" className='my-4 font-bold text-3xl'>{data.serviceLada.header}</p>}
				{!data.serviceLada.time && !data.serviceLada.coef && !data.serviceLada.model || <p className="my-4">Цена услуги: <span data-testid="price" className="font-bold text-orange-500">{((Number(data.serviceLada.time.split(",").join(".")) * Number(data.serviceLada.coef))*0.85).toFixed(0)} руб</span> (для Физ. лиц) Модель: <span data-testid="model" className="font-bold text-orange-500">{data.serviceLada.model}</span></p>}
				...
				<ServForm visibleModel={false} serv={data.serviceLada.header ? data.serviceLada.header:""} model={"Газель 4216"}></ServForm>
			</div>
		</Section>
	);
};