	...
	return (
		<Section color={"tint"}>
			<div className="my-12 mx-24 flex flex-col">
				{!data.serviceGaz4216.header || <p data-testid="header" className='my-4 font-bold text-3xl'>{data.serviceGaz4216.header}</p>}
				{!data.serviceGaz4216.time && !data.serviceGaz4216.coef && !data.serviceGaz4216.model || <p className="my-4">Цена услуги: <span data-testid="price" className="font-bold text-orange-500">{((Number(data.serviceGaz4216.time.split(",").join(".")) * Number(data.serviceGaz4216.coef))*0.85).toFixed(0)} руб</span> (для Физ. лиц) Модель: <span data-testid="model" className="font-bold text-orange-500">{data.serviceGaz4216.model}</span></p>}
