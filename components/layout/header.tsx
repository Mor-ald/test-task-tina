	...
	return (
		<div id="header_container" className={`relative overflow-hidden bg-gradient-to-b ${headerColorCss}`}>
			<div className="relative px-2 py-8 bg-[url('/fonsvet.jpg')] text-white md:py-4">
				<div className="flex flex-col justify-between w-4/5 mx-auto lg:flex-row">
					...
					<div className="flex items-start justify-center my-4">
						...
						<div className="ml-3">
							{data.address && data.address.map((item: {href: string, label: string}) => {
								return <p key={item.label}><a className="hover:opacity-60" href={`${item.href}`}>{item.label}</a></p>;
							})}
						</div>
					</div>
					{/* Address */}
					...
					<div className="flex items-start justify-center my-4">
						...
						<div className="ml-5">
							...
							<div className="mt-6 flex justify-center mr-8 md:mr-0 md:justify-start">
								<a href="https://vk.com/armstospb" className="transition duration-150 ease-out mr-2 hover:opacity-60" target="_blank" rel="noreferrer">
									<img src="/arm_vk.svg" alt="" />
								</a>
								<a href="https://instagram.com/armsto.spb" className="transition duration-150 ease-out mx-2 hover:opacity-60" target="_blank" rel="noreferrer">
									<img src="/arm_instagram.svg" alt="" />
								</a>
								<a href="https://api.whatsapp.com/send/?phone=79313996757&text&app_absent=0" className="transition duration-150 ease-out mx-2 hover:opacity-60" target="_blank" rel="noreferrer">
			...
			<Sidebar visible={visibleSidebar} onHide={() => setVisibleSidebar(false)} position={"right"}>
                ...
                data.nav.map((item) => {
                	return (
                		<div key={item.label} className="flex items-center ml-4 my-2 text-orange-600 font-bold text-lg transition duration-150 ease-out hover:opacity-60">
