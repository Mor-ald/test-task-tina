...
export const Header = ({ data }) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-b ${headerColorCss}`}>
      <div className="relative px-2 py-8 bg-[url('/fonsvet.jpg')] text-white">
        <div className="flex flex-col justify-between w-4/5 mx-auto md:flex-row">
          ...
          <div className="flex justify-center my-4">
            <a href={`mailto:${data.email}`} className="flex items-center py-4 px-6 mt-2 h-14 bg-orange-600">
              ...
              <span className="ml-3 font-bold text-sm text-white">{data.email}</span>
            </a>
          </div>
          {/* Social */}
      ...
      <div className="py-0 mt-4 relative w-4/5 mx-auto">
        <div className="flex items-center justify-center gap-6">
          <img src={`/${data.logo}`} width={370} alt="logo"/>
          <ul className="flex gap-6 sm:gap-8 lg:gap-10 tracking-[.002em] -mx-4 ml-3">
            {data.nav &&
              data.nav.map((item, i) => {
                const activeItem =
                  item.href === ""
