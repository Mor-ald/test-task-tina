	...
	return (
		<Section color={"tint"}>
			<div className="my-12 mx-24 flex flex-col">
				{!data.serviceSobol.header || <p data-testid="header" className='my-4 font-bold text-3xl'>{data.serviceSobol.header}</p>}
				{!data.serviceSobol.time && !data.serviceSobol.coef && !data.serviceSobol.model || <p className="my-4">Цена услуги: <span data-testid="price" className="font-bold text-orange-500">{((Number(data.serviceSobol.time.split(",").join(".")) * Number(data.serviceSobol.coef))*0.85).toFixed(0)} руб</span> (для Физ. лиц) Модель: <span data-testid="model" className="font-bold text-orange-500">{data.serviceSobol.model}</span></p>}
				...
				<ServForm visibleModel={false} serv={data.serviceSobol.header ? data.serviceSobol.header:""} model={"Газель 4216"}></ServForm>
			</div>
		</Section>
	);
};