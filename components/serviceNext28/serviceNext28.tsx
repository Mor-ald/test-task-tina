	...
	return (
		<Section color={"tint"}>
			<div className="my-12 mx-24 flex flex-col">
				{!data.serviceNext28.header || <p data-testid="header" className='my-4 font-bold text-3xl'>{data.serviceNext28.header}</p>}
				{!data.serviceNext28.time && !data.serviceNext28.coef && !data.serviceNext28.model || <p className="my-4">Цена услуги: <span data-testid="price" className="font-bold text-orange-500">{((Number(data.serviceNext28.time.split(",").join(".")) * Number(data.serviceNext28.coef))*0.85).toFixed(0)} руб</span> (для Физ. лиц) Модель: <span data-testid="model" className="font-bold text-orange-500">{data.serviceNext28.model}</span></p>}
				...
				<ServForm visibleModel={false} serv={data.serviceNext28.header ? data.serviceNext28.header:""} model={"Газель 4216"}></ServForm>
			</div>
		</Section>
	);
};