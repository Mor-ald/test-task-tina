	...
	return (
		<Section color={"tint"}>
			<div className="my-12 mx-24 flex flex-col">
				{!data.serviceNext27.header || <p data-testid="header" className='my-4 font-bold text-3xl'>{data.serviceNext27.header}</p>}
				{!data.serviceNext27.time && !data.serviceNext27.coef && !data.serviceNext27.model || <p className="my-4">Цена услуги: <span data-testid="price" className="font-bold text-orange-500">{((Number(data.serviceNext27.time.split(",").join(".")) * Number(data.serviceNext27.coef))*0.85).toFixed(0)} руб</span> (для Физ. лиц) Модель: <span data-testid="model" className="font-bold text-orange-500">{data.serviceNext27.model}</span></p>}
				...
				<ServForm visibleModel={false} serv={data.serviceNext27.header ? data.serviceNext27.header:""} model={"Газель 4216"}></ServForm>
			</div>
		</Section>
	);
};